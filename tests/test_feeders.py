...
TEST_FEEDERS_DIR = Path(__file__).parent.parent / "src" / "opendss_mcp" / "data" / "ieee_feeders"
...
EXPECTED_BUS_COUNTS = {
    'IEEE13': 15,   # Actual bus count in our simplified feeder
    'IEEE34': 8,    # Actual bus count in our simplified feeder  
    'IEEE123': 12   # Actual bus count in our simplified feeder
}

# Expected number of loads for each feeder
EXPECTED_LOAD_COUNTS = {
    'IEEE13': 15,   # Actual load count in our simplified feeder
    'IEEE34': 6,    # Actual load count in our simplified feeder
    'IEEE123': 9    # Actual load count in our simplified feeder
}
...
def test_power_flow(feeder_file):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dss.Solution.Summary()
        violations = dss.Solution.NumberofTimeSteps()
        assert violations >= 0, "Error in power flow solution"
