...
def test_full_der_study(ieee13_loaded):
    ...
    print(f"  - Voltage range: {baseline_min_voltage:.4f} - {baseline_max_voltage:.4f} pu")
    ...
    assert cap_result['success'], f"Capacity analysis failed: {cap_result.get('errors')}"
    ...
    load_profile = {
        ...
        "multipliers": [
            0.6 + 0.4 * np.sin((h - 6) * np.pi / 12) for h in hours
        ]
    }
    ...
    gen_profile = {
        "name": "test_solar",
        "multipliers": [
            max(0, np.sin((h - 6) * np.pi / 12)) for h in hours
        ]
    }
    ...
    assert ts_result['success'], f"Time-series simulation failed: {ts_result.get('errors')}"
    ...
    print(f"  - Voltage range: {summary.get('min_voltage_pu', 0):.4f} - {summary.get('max_voltage_pu', 0):.4f} pu")
    ...
    has_output = (
        viz_result['data'].get('file_path') is not None or
        viz_result['data'].get('image_base64') is not None
    )
    ...
    print(f"✓ Baseline power flow: {baseline_min_voltage:.4f}-{baseline_max_voltage:.4f} pu")
    ...
    print(f"✓ Time-series: {summary['num_timesteps']} timesteps, {summary.get('convergence_rate_pct', 0):.1f}% convergence")
...
def test_basic_workflow():
    ...
    print(f"✓ Voltage check completed ({vio_result['data']['summary']['total_violations']} violations)")
